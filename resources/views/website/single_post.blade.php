            ...
            <div class="row">
                ...
                <div class="col-md-8">
                    ...
                    <img style="width: 100%;height: 400px"
                         src="{{url('/')}}/public/uploads/post/{{$post->post_picture}}" alt="{{$post->post_title}}">
                    ...
                    <div class="col-sm-12 col-xs-12">
                        <figure class="wow fadeInLeft animated portfolio-item animated">
                            <div class="img-wrapper">
                                <img src="{{$youtube_video->snippet->thumbnails->high->url}}" class="img-responsive"
                                     alt="{{$youtube_video->snippet->title}}" width="100%">
                                <div class="overlay">
                                    <div class="buttons">
                                        <a target="_blank" data-youtube-id="{{$youtube_video->id}}"
                                           class="video-banner js-trigger-video-modal"
                                           href="http://www.youtube.com/watch?v={{$youtube_video->id}}">Play
                                            Video</a>
                                    </div>
                                </div>
                            </div>
                            <figcaption>
                                <h4>
                                    <a data-youtube-id="{{$youtube_video->id}}"
                                       class="video-banner js-trigger-video-modal" target="_blank"
                                       href="http://www.youtube.com/watch?v={{$youtube_video->id}}"> {{$youtube_video->snippet->title}}</a>
                                </h4>
                            </figcaption>
                        </figure>
                    </div>
...
">
                        <div class="col-md-9 col-xs-6">
                            <label> <span style="background-color: green"
                                                                     class="badge badge-light" id="total_like_count" ></span> <span id="total_like_count_text" onclick="likeSubmit()" style="cursor:pointer">Like</span></label>
                            </div>

                        <div class="col-md-3 col-xs-6"><label><span style="background-color: green"
                                                                    class="badge badge-light" id="total_single_comment"></span>Comments</label></div>
                    ...
                    <form id="myForm">
                        ...
                        <div class="form-group col-md-12">
                            <button type="button" onclick="Comment_submit({{$post->post_id}})"
    ...
    <script defer>
        ...
        function allCommentCount() {
            var post_id = $('#post_id_id').val();
            $.ajax({
                url: "{{url('/')}}/all_comment_count/"+post_id,

                success: function (data) {
                 $('#total_single_comment').text(data)
                }
            });
        }
        ...
        function allLikeCount() {
            var post_id = $('#post_id_id').val();
            $.ajax({
                url: "{{url('/')}}/all_like_count/"+post_id,

                success: function (data) {
                    console.log(data)
                    $('#total_like_count').text(data.total_like)
                    if(data.like==1){
                        $('#total_like_count_text').html("<strong><span style='color:green'>Like</span></strong>")
                    } else {
                        $('#total_like_count_text').html("<strong><span style='color:black'>Like</span></strong>")
        ...
        function likeSubmit() {
            var post_id = $('#post_id_id').val();
            $.ajax({
                url: "{{url('/')}}/all_like_submit/"+post_id,
